...
**Correction (같은 날 늦게):** 위 불일치는 이후 Gemini가 프론트를 실제 `/analytics/stats` 계약(4개 지표: 총 순방문자/로그인 교직원/신규가입/신규학원)에 맞춰 재작성하며 해소됨 — `admin-service.ts`의 `generateMockVisitorData`는 완전 제거, `POST /analytics/track` beacon도 `AppLayout.tsx`/로그인 페이지에 실연동됨. `AI_HANDOFF.md` 참고.

**Feature: 역할별 커스텀 권한 (Role Permissions) (2026-09-08):** 사용자 요청 — "강사/조교 권한을 원장이 교직원 관리에서 커스텀할 수 있으면 좋겠다, 초기값은 최소 권한(전부 보기만)". `superpowers:brainstorming`(질문 도중 사용자가 방향을 개별 직원별 → **역할(실장/강사/조교) 단위 템플릿**으로 피벗)→`writing-plans`→`executing-plans` 플로우로 진행(스펙: `docs/superpowers/specs/2026-09-08-role-permissions-design.md`, 플랜: `docs/superpowers/plans/2026-09-08-role-permissions.md`). 신규 `RolePermission` 테이블(학원×역할×메뉴별 `canEdit`) + 지연 기본값 폴백(학원마다 24행 미리 만들 필요 없음). 신규 `PermissionGuard`+`@RequirePermission()` 데코레이터를 기존 8개 컨트롤러(students/classes/attendance/class-logs/tuition/calendar/notifications/reports)의 총 25개 쓰기 라우트에 적용 — 기존 `@Roles()`는 그대로 두고 그 안에서 "수정 가능 여부"만 추가로 좁히는 하위 게이트로 설계(보기 권한은 절대 안 넓어짐). `PermissionsModule`을 `PrismaModule`과 동일하게 `@Global()`로 만들어 8개 모듈이 각자 import 안 해도 가드가 해석되게 함. 기본값(ADMIN 전 메뉴 수정 가능=기존 동작 그대로, TEACHER는 출결·수업일지·리포트만 수정 가능, STAFF는 출결만)은 실제 기존 `@Roles()` 코드를 전수 확인해서 도출(추측 아님 — 예: 출결은 조교도 이미 키오스크 보조로 실무 중이라 확인 후 기본값에 반영). 신규 `GET/PATCH /auth/role-permissions`(조회는 원장+실장, 변경은 원장 전용). **실행 중 계획 문서에 없던 결함 발견 및 수정**: `students.controller.spec.ts`/`classes.controller.spec.ts`가 `PermissionsModule`을 import하지 않는 격리된 `TestingModule`을 쓰고 있어서 `PermissionGuard`의 DI가 깨짐 — `PermissionsService` 목 프로바이더 추가로 해결(플랜의 "이 저장소는 컨트롤러 spec이 없다"는 전제가 틀렸던 걸 실행 중 확인). 테스트: `permissions.service.spec.ts` 8개, `permission.guard.spec.ts` 6개 신규(전체 스위트 169). 문서: `docs/domains/09-role-permissions.md` 신규.
